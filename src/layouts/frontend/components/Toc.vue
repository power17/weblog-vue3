<template>
  ...
  >
    ...
    <div class="toc-wrapper">
      <ul class="toc">
        <!-- 二级标题 -->
        <li class="cursor-pointer" v-for="(h2, index) in titles" :key="index">
          <span
          ...
          <ul v-if="h2.children && h2.children.length > 0">
            <li class="cursor-pointer" v-for="(h3, index2) in h2.children" :key="index2">
              <span
...
// 滚动到指定的位置
function scrollToView(offsetTop: number) {
  window.scrollTo({ top: offsetTop, behavior: 'smooth' })
}
onMounted(() => {
  // 通过 .artilce-content 样式来获取父级 div
  const container: HTMLDivElement | null = document.querySelector('.article-content')
...
const initTocData = (container: HTMLDivElement | null) => {
  // 只提取二级、三级标题
  let levels = 'h2,h3'
  let headings: NodeListOf<HTMLElement> | undefined = container?.querySelectorAll(levels)
...
function handleContentScroll() {
  ...
  titles.value.forEach((title) => {
    ...
    if (children && children.length > 0) {
      children.forEach((child: { offsetTop: any; index: number }) => {
