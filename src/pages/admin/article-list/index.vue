<template>
  <div>
    ...
    <el-card shadow="never">
      <!-- 写文章按钮 -->
      <div class="mb-5">
        <el-button @click="isArticlePublishEditorShow = true" type="primary">
          <el-icon class="mr-1">
            <EditPen />
          </el-icon>
          写文章</el-button
            ...
            <el-affix :offset="20" style="width: 100%">
              ...
              <div class="flex h-10 bg-white">
                ...
                <div class="ml-auto flex">
                  ...
                  <el-button @click="publishArticleSubmit" type="primary">
                    <el-icon class="mr-1">
                      <Promotion />
                    </el-icon>
                    发布
    ...
    >
      <template #header="{ close, titleId, titleClass }">
        <!-- 固钉组件，固钉到顶部 -->
        <el-affix class="bg-white" :offset="20" style="width: 100%">
          <!-- 指定 flex 布局， 高度为 10， 背景色为白色 -->
          <div class="flex h-10 pt-[20px] bg-white">
            <!-- 字体加粗 -->
...
const size = ref(10)
// 发布文章表单引用
const publishArticleFormRef = ref()
// 是否显示编辑文章对话框
const isArticleUpdateEditorShow = ref(false)
// 编辑文章表单引用
const updateArticleFormRef = ref()
// 编辑文章表单引用
// 文章标签
...
getTagSelectList().then((res) => {
  tags.value = res.data as TagsType[]
})
// 修改文章表单对象
...
// 编辑文章按钮点击事件
const showArticleUpdateEditor = (row: { id: string }) => {
  // 显示编辑文章对话框
  ...
  if (query) {
    ...
    searchTags(query)
      .then((e) => {
        if (e.success) {
          // 设置到 tags 变量中
          tags.value = e.data as { value: string; label: string }[]
        }
      })
...
// 删除文章
const deleteArticleSubmit = (row: { id: string }) => {
...
// 上传文章封面图片
const handleCoverChange = (file: ElUploadFile) => {
  // 表单对象
  let formData = new FormData()
  // 添加 file 字段，并将文件传入
  formData.append('file', file.raw || new Blob())
  uploadFile(formData).then((e) => {
    // 响参失败，提示错误消息
    if (e.success == false) {
      let message = e.message
...
getCategorySelectList().then((e) => {
  console.log('获取分类数据')
  categories.value = e.data as categoriesType[]
})
// 重置
const reset = () => {
  pickDate.value = ''
